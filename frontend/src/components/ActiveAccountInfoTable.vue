<template>
	<v-layout class="dashboard__active-account__info-table" align-center row wrap>
		<template v-for="item, index in accountInfo">
			<v-flex xs6 class="active-account__info-row left-column">
				<v-chip
					disabled
					color="primary">
			...
			<v-flex xs6 class="active-account__info-row right-column">
				<v-chip
					disabled
					class="chip-content"
		...
		computed: {
			...
			accountInfo() {
				const formatMoney = (balance) => {
					return parseFloat(balance).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$& ')
				}
