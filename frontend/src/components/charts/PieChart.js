...
export default {
	...
	methods: {
		getDataCollection() {
			const expensesTotalAmount = _.sum(_.map(this.expenses, (expense) => {
				return expense.amount
			})).toFixed(2)
			const earningsTotalAmount = _.sum(_.map(this.earnings, (earnings) => {
			...
			console.log(earningsTotalAmount, expensesTotalAmount)
